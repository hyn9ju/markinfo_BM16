...
function openMbtiGuide(){
  ...
  $('mmRows').innerHTML=Object.keys(STYLES).map(code=>{
    ...
    return `<tr data-code="${code}" class="${code===curCode?'cur':''}">
      ...
      <td class="mm-adj">${st.adj}<small>${st.sub}</small></td>
...
/* ================= 업종 코드 + 목업 (canvas 합성) ================= */
const BIZ_LIST=[["fnb","F&B"],["beauty","뷰티·코스메틱"],["fashion","패션"],["life","라이프스타일"],
 ["tech","테크·IT"],["finance","금융·핀테크"],["medical","의료·헬스케어"],["edu","교육"],
 ["retail","유통·리테일"],["mfg","제조·산업"],["estate","건축·부동산"],["pro","전문서비스"],
 ["media","미디어·문화·엔터테인먼트"],["travel","여행·숙박"],["public","공공·비영리"]];
...
function initBizChips(){
  ...
  box.innerHTML=BIZ_LIST.map(([k,l])=>`<button type="button" class="biz-chip${k===bizCode?' on':''}" data-biz="${k}">${l}</button>`).join('');
...
function bindMockupUI(){
  ...
  if(!sel.dataset.ready){
    sel.innerHTML='<option value="">공통 (업종 미선택)</option>'+BIZ_LIST.map(([k,l])=>`<option value="${k}">${l}</option>`).join('')+'<option value="_online">온라인 · SNS 프로필</option>';
    sel.dataset.ready='1';
      ...
      r.onload=()=>{
        ...
        im.onload=()=>{
          userLogo={src:r.result,img:im,scale:(+$('ulScale').value||100)/100,name:f.name,
                    vector:/svg/i.test(f.type)||/\.svg$/i.test(f.name)};
...
function renderLogoLayer(o){
  ...
  if(o.img){                                   // 업로드 로고 이미지
    ...
    const t=o.text||''; const fw=o.fw||700;
    let fs=Math.floor(H*.75);
    const set=()=>x.font=`${fw} ${fs}px ${o.fam}, Pretendard, sans-serif`;
    set(); while(x.measureText(t).width>W*.98&&fs>8){fs-=2;set();}
    x.fillStyle=o.color||'#111';
    x.textAlign=al; x.textBaseline='middle';
    x.fillText(t, al==='left'?0:al==='right'?W:W/2, H/2);
  }
  return cv;
}
...
<svg xmlns="http://www.w3.org/2000/svg" width="${tw}" height="${th}" viewBox="0 0 ${tw} ${th}">
  ...
  if(kind==='svg-outline'){
    ...
    const d=loops.map(lp=>{
      const sp=rdp(lp,0.7);
      return 'M'+sp.map((p,i)=>(i?'L':'')+r2(p[0])+' '+r2(p[1])).join(' ')+'Z';
    }).join(' ');
...
async function drawMockup(){
  ...
  if(scene==='online'){ /* 온라인 · SNS 프로필 목업 */
    ...
    const drawLogo=()=>{
      ...
      compositeLogo(x,layer,p,base.warp, userLogoActive()?'normal':bl, userLogoActive()?1:.94);
    };
    if(isLayer){
...
function amDraw(){
  ...
  if(lmOn){
    ...
    if(!box){
      ...
      const light=yiq(bgc)>150; bl=light?'multiply':'screen'; logoC=light?'#22302C':'#FFFFFF';
      drawLogo();                                                                // ③ 로고
...
/* ================= 상담결과 불러오기 ================= */
function loadBrief(){
  const t=($('briefInput').value||'');
  const m=t.match(/([GL])\s*[·.]?\s*([BS])\s*[·.]?\s*([ER])\s*[·.]?\s*([CM])/);
  if(!m){toast('요약에서 스타일 코드를 찾지 못했어요 — 복사한 내용을 그대로 붙여 넣어 주세요');return;}
  const code=m[1]+m[2]+m[3]+m[4];
  if(!STYLES[code]){toast('알 수 없는 스타일 코드입니다: '+code);return;}
  const STRV={'약간':1,'보통':2,'강함':3,'뚜렷하게':2,'강하게':3};
  [...code].forEach((ch,i)=>{
    const ax=AXES[i], side=ax.L.c===ch?-1:1;
    const re=new RegExp(ax.name+'\\s*:\\s*\\S+\\((약간|보통|강함|뚜렷하게|강하게)\\)');
    const mm=t.match(re);
    const str=mm?STRV[mm[1]]:2;
    sliders[i].value=3+side*str;
  });
  showApp();switchTab('S');render();
  toast('상담 결과를 불러왔어요: '+STYLES[code].adj);
...
function initAdminSelects(){
  ...
  if(!sel.options.length){
    Object.entries(STYLES).forEach(([c,st])=>{const o=document.createElement('option');o.value=c;o.textContent=`${st.adj} (${c})`;sel.appendChild(o);});
    if(curCode)sel.value=curCode;
  }
  ...
  if(isel&&!isel.options.length){
    Object.entries(STYLES).forEach(([c,st])=>{const o=document.createElement('option');o.value=c;o.textContent=`${st.adj} (${c})`;isel.appendChild(o);});
    if(curCode)isel.value=curCode;
  }
}
...
function renderFontList(keep){
  ...
  $('fStat').textContent=`${list.length}종 표시${list.length>show.length?` (${show.length}종 렌더)`:''} · 전체 ${FONT_DB.length}종`;
