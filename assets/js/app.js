...
async function exportSiteZip(){
  ...
  for(const rel of [...STATIC,...new Set(EXIST_IMGS)]){
    try{
      const r=await fetch(rel,{cache:'no-store'}); if(!r.ok){fetchFail++;continue;}
      zip.file(root+'/'+rel, await r.arrayBuffer());
    }catch(e){fetchFail++;}
  }
  // app.js는 버전 표시 상수를 새 버전으로 맞춰서 담는다
  try{
    const r=await fetch('assets/js/app.js',{cache:'no-store'});
    let code=await r.text();
    code=code.replace(/let APP_VERSION='[\d.]+';/, `let APP_VERSION='${newVer}';`)
             .replace(/const BUILD_DATE='[\d-]+';/, `const BUILD_DATE='${today}';`);
    zip.file(root+'/assets/js/app.js', code);
  }catch(e){ alert('app.js를 담지 못했어요 — 내보내기를 중단합니다.'); return; }
  ...
  clearDirty();
  toast(`v${newVer} 웹 업로드용 폴더(.zip)를 내려받았어요`+(fetchFail?` · 일부 파일 ${fetchFail}개는 담지 못했어요`:''));
}
